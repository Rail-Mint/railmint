...
const tonePresets = [
	{
		id: "strategist",
		label: "Strategist",
		lead: "A sharp BNB ecosystem strategist",
	},
	{ id: "builder", label: "Builder", lead: "A practical Web3 builder" },
	{
		id: "educator",
		label: "Educator",
		lead: "A clear and trusted crypto educator",
	},
] as const;
...
export function StudioOnboarding({ address, onComplete }: Props) {
	...
	const profile = useMemo(() => {
		const tone =
			tonePresets.find((t) => t.id === selectedTone) ?? tonePresets[0];
		const focus =
			focusPresets.find((f) => f.id === selectedFocus) ?? focusPresets[0];
		const goal =
			goalPresets.find((g) => g.id === selectedGoal) ?? goalPresets[0];
		const note = customNote.trim()
			? ` Keep this nuance: ${customNote.trim()}.`
			: "";
		...
		(async () => {
			...
			if (!cancelled && data) {
				toast({
					...
					description: "Redirecting to your studio...",
				});
				onComplete();
			}
		})();
	...
	const handleIdentityNext = useCallback(async () => {
		...
		try {
			...
			if (nameRes.data) {
				toast({
					title: "Clone name taken",
					description: `"${cloneName}" is already in use. Choose a different name.`,
					variant: "destructive",
				});
				return;
			}
			...
			toast({
				title: "Check failed",
				description: "Could not verify uniqueness. Try again.",
				variant: "destructive",
			});
		} finally {
	...
	const handleCreate = useCallback(async () => {
		...
		try {
			...
			toast({
				title: "Clone created!",
				description: `${values.clone_name} is ready.`,
			});
			setStep(4);
		} catch (err: any) {
			const msg = (err.message || "").toLowerCase();
			if (
				msg.includes("clone name is already") ||
				msg.includes("name is already taken")
			) {
				toast({
					title: "Name conflict",
					description:
						"This clone name was just taken. Choose a different name.",
					variant: "destructive",
				});
				setStep(1);
				setSaving(false);
			...
			<div className="flex items-center gap-2">
				{steps.map((s, i) => (
					<div key={s.id} className="flex items-center gap-2">
						...
						<span
							className={`text-sm hidden sm:inline ${step >= s.id ? "text-foreground" : "text-muted-foreground"}`}
						>
							{s.label}
						</span>
						...
						<Card className="border-border/40">
							...
							<CardContent className="space-y-4">
								<div>
									...
									<Input
										{...form.register("clone_name")}
										className="border-border/40"
									/>
									{form.formState.errors.clone_name && (
										<p className="mt-1 text-xs text-destructive">
											{form.formState.errors.clone_name.message}
										</p>
									)}
								</div>
								<Button
					...
					>
						<Card className="border-border/40">
							...
							<CardContent className="space-y-4">
								<ReviewField
									label="Clone Name"
									value={form.getValues("clone_name")}
								/>
						...
						<Card className="border-primary/30 bg-gradient-to-br from-card to-primary/[0.03]">
							...
							<CardContent className="space-y-4 text-center">
								<p className="text-sm text-muted-foreground">
									Connect your X account via OAuth to prove you own it. This
									links your wallet to your real X identity.
								</p>
