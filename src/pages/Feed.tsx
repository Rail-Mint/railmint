...
export default function Feed() {
  ...
  async function loadData() {
    ...
    const enriched: Post[] = postsData.map((post) => ({
      ...
      liked_by_me: address
        ? likes.some((like) => like.post_id === post.id && like.wallet_address === address)
        : false,
    }));
  ...
  const sortedPosts = [...posts].sort((a, b) => {
    if (trustFirst) {
      const trustDelta = Number(Boolean(b.commit_tx_hash)) - Number(Boolean(a.commit_tx_hash));
    ...
    <div className="container py-6 sm:py-8 md:py-10">
      ...
      {sortedPosts.length === 0 ? (
        ...
        <>
                  ...
                  <CardHeader className="relative border-b border-border/60 pb-3">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-1.5">
                        ...
                        {post.is_fallback ? <Badge variant="secondary" className="text-[11px]">Fallback</Badge> : null}
                      </div>
                    </div>
                  </CardHeader>
          ...
          <div className="mt-6 flex items-center justify-between gap-3 rounded-xl border border-border/70 bg-background/70 px-3 py-2">
