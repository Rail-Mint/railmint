...
export default function Onboarding() {
	...
	const form = useForm<FormValues>({
		...
		defaultValues: {
			...
			persona_text: "",
			prompt_template:
				"Write a 200-word post about {{topic}} in the BNB ecosystem. Be informative and engaging.",
		},
	});
	...
	const handleSave = async () => {
		...
		setLoading(true);
		try {
			const handle = savedData.x_handle.startsWith("@")
				? savedData.x_handle
				: `@${savedData.x_handle}`;
	...
	if (step === "done") {
		return (
			<div className="container px-4 py-14 text-center sm:py-20">
				...
				<h1 className="mb-4 text-2xl font-bold sm:text-3xl">Clone Ready!</h1>
				<p className="text-muted-foreground mb-2">
					Your AI clone <strong>{savedData?.clone_name}</strong> is set up.
				</p>
				<p className="text-sm text-muted-foreground">
					Head to the feed to see generated content.
				</p>
			</div>
		);
	}
	...
	if (step === "preview" && savedData) {
		return (
			<div className="container mx-auto max-w-lg px-4 py-8 sm:py-12">
				<h1 className="mb-6 text-2xl font-bold">Preview Your Clone</h1>
				<Card>
					<CardHeader>
	...
	return (
		<div className="container mx-auto max-w-lg px-4 py-8 sm:py-12">
			<h1 className="mb-6 text-2xl font-bold">Create Your AI Clone</h1>
			<Card>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Input placeholder="BNB Sage" {...field} />
										</FormControl>
										<FormMessage />
									</FormItem>
								)}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Textarea
												placeholder="Describe your clone's personality, writing style, and areas of expertise…"
												rows={4}
												{...field}
											/>
										</FormControl>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Textarea
												placeholder="Use {{topic}} as a placeholder for the content topic"
												rows={3}
												{...field}
											/>
										</FormControl>
